...
const shouldIgnoreError = (errorText: string) =>
  ERRORS_TO_IGNORE.some((regex) => regex.test(errorText));
...
const getSentryIssue = async (
  eventId: string,
  tries = 0
): Promise<SentryIssue | undefined | void> => {
  ...
  try {
    ...
    const response = await fetch(`https://kap-sentry-tracker.vercel.app/api/event/${eventId}`);
